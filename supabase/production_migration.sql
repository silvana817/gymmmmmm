...
-- =============================================================================
-- Alignment for installations that already ran the legacy schema
-- =============================================================================
...
ALTER TABLE public.nutricion_registros
    ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ;
...
SET nombre = btrim(nombre),
    ...
    plan = NULLIF(btrim(plan), ''),
    avatar = NULLIF(btrim(avatar), ''),
    objetivo = NULLIF(btrim(objetivo), ''),
    created_at = COALESCE(created_at, NOW()),
    updated_at = COALESCE(updated_at, created_at, NOW());

UPDATE public.rutinas
SET nombre = btrim(nombre),
    descripcion = NULLIF(btrim(descripcion), ''),
    tipo = NULLIF(btrim(tipo), ''),
    creador = NULLIF(btrim(creador), ''),
    dias = COALESCE(dias, '[]'::jsonb),
    asignaciones = COALESCE(asignaciones, '[]'::jsonb),
    created_at = COALESCE(created_at, NOW()),
    updated_at = COALESCE(updated_at, created_at, NOW());

UPDATE public.pagos
SET mes = btrim(mes),
    metodo = NULLIF(btrim(metodo), ''),
    created_at = COALESCE(created_at, NOW()),
    updated_at = COALESCE(updated_at, created_at, NOW());
...
SET nombre = btrim(nombre),
    descripcion = NULLIF(btrim(descripcion), ''),
    comidas = COALESCE(comidas, '[]'::jsonb),
    created_at = COALESCE(created_at, NOW()),
    updated_at = COALESCE(updated_at, created_at, NOW());

UPDATE public.nutricion_perfiles
SET meta_diaria = CASE
        WHEN meta_diaria IS NULL OR NOT public.is_valid_nutrition_meta(meta_diaria)
            THEN public.default_nutrition_meta()
        ELSE meta_diaria
    END,
    created_at = COALESCE(created_at, NOW()),
    updated_at = COALESCE(updated_at, created_at, NOW());

UPDATE public.nutricion_registros
SET comidas = COALESCE(comidas, '[]'::jsonb),
    created_at = COALESCE(created_at, NOW()),
    updated_at = COALESCE(updated_at, created_at, NOW());
...
SET completed = COALESCE(completed, FALSE),
    ...
    actual_reps = NULLIF(btrim(actual_reps), ''),
    actual_weight_kg = NULLIF(btrim(actual_weight_kg), ''),
    completed_at = CASE
        WHEN COALESCE(completed, FALSE) THEN COALESCE(completed_at, created_at, NOW())
        ELSE NULL
    END,
    created_at = COALESCE(created_at, NOW()),
    updated_at = COALESCE(updated_at, created_at, NOW());

UPDATE public.pagos pago
SET tenant_id = alumno.tenant_id
FROM public.alumnos alumno
WHERE alumno.id = pago.alumno_id
  AND pago.tenant_id IS DISTINCT FROM alumno.tenant_id;

UPDATE public.nutricion_perfiles perfil
SET tenant_id = alumno.tenant_id
FROM public.alumnos alumno
WHERE alumno.id = perfil.alumno_id
  AND perfil.tenant_id IS DISTINCT FROM alumno.tenant_id;

UPDATE public.nutricion_registros registro
SET tenant_id = alumno.tenant_id
FROM public.alumnos alumno
WHERE alumno.id = registro.alumno_id
  AND registro.tenant_id IS DISTINCT FROM alumno.tenant_id;

UPDATE public.workout_logs log
SET tenant_id = alumno.tenant_id
FROM public.alumnos alumno
WHERE alumno.id = log.alumno_id
  AND log.tenant_id IS DISTINCT FROM alumno.tenant_id;

UPDATE public.nutricion_perfiles perfil
SET plan_activo_id = NULL
WHERE plan_activo_id IS NOT NULL
  AND NOT EXISTS (
      SELECT 1
      FROM public.planes_nutricionales plan
      WHERE plan.id = perfil.plan_activo_id
        AND plan.tenant_id = perfil.tenant_id
  );
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Migration aborted: tenant_users contains multiple rows for the same user_id. Resolve duplicates before re-running.';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Migration aborted: alumnos contains multiple rows for the same user_id. Resolve duplicates before re-running.';
    END IF;

    IF EXISTS (
        SELECT 1
        FROM public.workout_logs log
        JOIN public.alumnos alumno ON alumno.id = log.alumno_id
        JOIN public.rutinas rutina ON rutina.id = log.rutina_id
        WHERE alumno.tenant_id <> rutina.tenant_id
    ) THEN
        RAISE EXCEPTION 'Migration aborted: workout_logs references alumnos and rutinas from different tenants.';
    END IF;
END
$$;
...
ALTER TABLE public.tenants
    ADD CONSTRAINT tenants_name_not_blank CHECK (btrim(name) <> '');

ALTER TABLE public.tenant_users
    DROP CONSTRAINT IF EXISTS tenant_users_role_check,
    DROP CONSTRAINT IF EXISTS tenant_users_user_unique,
...
ALTER TABLE public.tenant_users
    ...
    USING (
        CASE lower(btrim(role::text))
            WHEN 'entrenador' THEN 'entrenador'::public.app_role
            ELSE 'alumno'::public.app_role
        END
    );
...
ALTER TABLE public.tenant_users
    ADD CONSTRAINT tenant_users_user_unique UNIQUE (user_id);

ALTER TABLE public.alumnos
    DROP CONSTRAINT IF EXISTS alumnos_estado_check,
    DROP CONSTRAINT IF EXISTS alumnos_nombre_not_blank,
    DROP CONSTRAINT IF EXISTS alumnos_email_not_blank,
    DROP CONSTRAINT IF EXISTS alumnos_telefono_not_blank,
    DROP CONSTRAINT IF EXISTS alumnos_plan_not_blank,
    DROP CONSTRAINT IF EXISTS alumnos_avatar_not_blank,
    DROP CONSTRAINT IF EXISTS alumnos_objetivo_not_blank,
    DROP CONSTRAINT IF EXISTS alumnos_peso_positive,
    DROP CONSTRAINT IF EXISTS alumnos_altura_positive,
    DROP CONSTRAINT IF EXISTS alumnos_edad_valid,
    DROP CONSTRAINT IF EXISTS alumnos_tenant_id_id_unique,
    DROP CONSTRAINT IF EXISTS alumnos_tenant_id_user_id_key;
...
ALTER TABLE public.alumnos
    ALTER COLUMN estado TYPE public.alumno_estado
    USING (
        CASE lower(btrim(estado::text))
            WHEN 'vencido' THEN 'vencido'::public.alumno_estado
            WHEN 'inactivo' THEN 'inactivo'::public.alumno_estado
            ELSE 'activo'::public.alumno_estado
        END
    );
...
ALTER TABLE public.alumnos
    ...
    ALTER COLUMN peso TYPE NUMERIC(6, 2) USING CASE WHEN peso IS NULL THEN NULL ELSE ROUND(peso::numeric, 2) END,
    ALTER COLUMN altura TYPE NUMERIC(5, 2) USING CASE WHEN altura IS NULL THEN NULL ELSE ROUND(altura::numeric, 2) END;

ALTER TABLE public.alumnos
    ADD CONSTRAINT alumnos_nombre_not_blank CHECK (btrim(nombre) <> ''),
    ADD CONSTRAINT alumnos_email_not_blank CHECK (email IS NULL OR btrim(email) <> ''),
    ADD CONSTRAINT alumnos_telefono_not_blank CHECK (telefono IS NULL OR btrim(telefono) <> ''),
    ADD CONSTRAINT alumnos_plan_not_blank CHECK (plan IS NULL OR btrim(plan) <> ''),
    ADD CONSTRAINT alumnos_avatar_not_blank CHECK (avatar IS NULL OR btrim(avatar) <> ''),
    ADD CONSTRAINT alumnos_objetivo_not_blank CHECK (objetivo IS NULL OR btrim(objetivo) <> ''),
    ADD CONSTRAINT alumnos_peso_positive CHECK (peso IS NULL OR peso > 0),
    ADD CONSTRAINT alumnos_altura_positive CHECK (altura IS NULL OR altura > 0),
    ADD CONSTRAINT alumnos_edad_valid CHECK (edad IS NULL OR edad BETWEEN 0 AND 120),
    ADD CONSTRAINT alumnos_tenant_id_id_unique UNIQUE (tenant_id, id);

ALTER TABLE public.rutinas
    DROP CONSTRAINT IF EXISTS rutinas_nombre_not_blank,
    DROP CONSTRAINT IF EXISTS rutinas_descripcion_not_blank,
    DROP CONSTRAINT IF EXISTS rutinas_tipo_not_blank,
    DROP CONSTRAINT IF EXISTS rutinas_creador_not_blank,
    DROP CONSTRAINT IF EXISTS rutinas_dias_is_array,
    DROP CONSTRAINT IF EXISTS rutinas_asignaciones_is_array,
    DROP CONSTRAINT IF EXISTS rutinas_tenant_id_id_unique;

ALTER TABLE public.rutinas
    ALTER COLUMN nombre SET NOT NULL,
    ALTER COLUMN dias SET DEFAULT '[]'::jsonb,
    ALTER COLUMN asignaciones SET DEFAULT '[]'::jsonb,
    ALTER COLUMN dias SET NOT NULL,
    ALTER COLUMN asignaciones SET NOT NULL,
    ALTER COLUMN created_at SET DEFAULT NOW(),
    ALTER COLUMN updated_at SET DEFAULT NOW(),
    ALTER COLUMN created_at SET NOT NULL,
    ALTER COLUMN updated_at SET NOT NULL;
...
ALTER TABLE public.rutinas
    ADD CONSTRAINT rutinas_nombre_not_blank CHECK (btrim(nombre) <> ''),
    ADD CONSTRAINT rutinas_descripcion_not_blank CHECK (descripcion IS NULL OR btrim(descripcion) <> ''),
    ADD CONSTRAINT rutinas_tipo_not_blank CHECK (tipo IS NULL OR btrim(tipo) <> ''),
    ADD CONSTRAINT rutinas_creador_not_blank CHECK (creador IS NULL OR btrim(creador) <> ''),
    ADD CONSTRAINT rutinas_dias_is_array CHECK (public.jsonb_is_array(dias)),
    ADD CONSTRAINT rutinas_asignaciones_is_array CHECK (public.jsonb_is_array(asignaciones)),
    ADD CONSTRAINT rutinas_tenant_id_id_unique UNIQUE (tenant_id, id);

ALTER TABLE public.pagos
    DROP CONSTRAINT IF EXISTS pagos_estado_check,
    DROP CONSTRAINT IF EXISTS pagos_monto_non_negative,
    DROP CONSTRAINT IF EXISTS pagos_mes_not_blank,
    DROP CONSTRAINT IF EXISTS pagos_metodo_not_blank,
    DROP CONSTRAINT IF EXISTS pagos_alumno_same_tenant_fkey,
    DROP CONSTRAINT IF EXISTS pagos_alumno_id_fkey;
...
ALTER TABLE public.pagos
    ...
    USING (
        CASE lower(btrim(estado::text))
            WHEN 'pagado' THEN 'pagado'::public.pago_estado
            WHEN 'vencido' THEN 'vencido'::public.pago_estado
            WHEN 'cancelado' THEN 'cancelado'::public.pago_estado
            ELSE 'pendiente'::public.pago_estado
        END
    );
...
ALTER TABLE public.pagos
    ADD CONSTRAINT pagos_monto_non_negative CHECK (monto >= 0),
    ADD CONSTRAINT pagos_mes_not_blank CHECK (btrim(mes) <> ''),
    ADD CONSTRAINT pagos_metodo_not_blank CHECK (metodo IS NULL OR btrim(metodo) <> '');

ALTER TABLE public.pagos
    ADD CONSTRAINT pagos_alumno_same_tenant_fkey FOREIGN KEY (tenant_id, alumno_id)
        REFERENCES public.alumnos(tenant_id, id) ON DELETE CASCADE;

ALTER TABLE public.planes_nutricionales
    DROP CONSTRAINT IF EXISTS planes_nutricionales_nombre_not_blank,
    DROP CONSTRAINT IF EXISTS planes_nutricionales_descripcion_not_blank,
    DROP CONSTRAINT IF EXISTS planes_nutricionales_comidas_is_array,
    DROP CONSTRAINT IF EXISTS planes_nutricionales_tenant_id_id_unique;

ALTER TABLE public.planes_nutricionales
    ALTER COLUMN nombre SET NOT NULL,
    ALTER COLUMN comidas SET DEFAULT '[]'::jsonb,
    ALTER COLUMN comidas SET NOT NULL,
    ALTER COLUMN created_at SET DEFAULT NOW(),
    ALTER COLUMN updated_at SET DEFAULT NOW(),
    ALTER COLUMN created_at SET NOT NULL,
    ALTER COLUMN updated_at SET NOT NULL;
...
ALTER TABLE public.planes_nutricionales
    ADD CONSTRAINT planes_nutricionales_nombre_not_blank CHECK (btrim(nombre) <> ''),
    ADD CONSTRAINT planes_nutricionales_descripcion_not_blank CHECK (descripcion IS NULL OR btrim(descripcion) <> ''),
    ADD CONSTRAINT planes_nutricionales_comidas_is_array CHECK (public.jsonb_is_array(comidas)),
    ADD CONSTRAINT planes_nutricionales_tenant_id_id_unique UNIQUE (tenant_id, id);

ALTER TABLE public.nutricion_perfiles
    DROP CONSTRAINT IF EXISTS nutricion_perfiles_meta_diaria_valid,
    DROP CONSTRAINT IF EXISTS nutricion_perfiles_alumno_same_tenant_fkey,
    DROP CONSTRAINT IF EXISTS nutricion_perfiles_tenant_alumno_unique,
    DROP CONSTRAINT IF EXISTS nutricion_perfiles_alumno_id_fkey,
    DROP CONSTRAINT IF EXISTS nutricion_perfiles_plan_activo_id_fkey;

ALTER TABLE public.nutricion_perfiles
    ALTER COLUMN meta_diaria SET DEFAULT public.default_nutrition_meta(),
    ALTER COLUMN meta_diaria SET NOT NULL,
    ALTER COLUMN created_at SET DEFAULT NOW(),
    ALTER COLUMN updated_at SET DEFAULT NOW(),
    ALTER COLUMN created_at SET NOT NULL,
    ALTER COLUMN updated_at SET NOT NULL;
...
ALTER TABLE public.nutricion_perfiles
    ADD CONSTRAINT nutricion_perfiles_meta_diaria_valid CHECK (public.is_valid_nutrition_meta(meta_diaria)),
    ADD CONSTRAINT nutricion_perfiles_alumno_same_tenant_fkey FOREIGN KEY (tenant_id, alumno_id)
        REFERENCES public.alumnos(tenant_id, id) ON DELETE CASCADE,
    ADD CONSTRAINT nutricion_perfiles_tenant_alumno_unique UNIQUE (tenant_id, alumno_id),
    ADD CONSTRAINT nutricion_perfiles_plan_activo_id_fkey FOREIGN KEY (plan_activo_id)
        REFERENCES public.planes_nutricionales(id) ON DELETE SET NULL;

ALTER TABLE public.nutricion_registros
    DROP CONSTRAINT IF EXISTS nutricion_registros_comidas_is_array,
    DROP CONSTRAINT IF EXISTS nutricion_registros_alumno_same_tenant_fkey,
    DROP CONSTRAINT IF EXISTS nutricion_registros_alumno_id_fkey;
...
ALTER TABLE public.nutricion_registros
    ADD CONSTRAINT nutricion_registros_comidas_is_array CHECK (public.jsonb_is_array(comidas)),
    ADD CONSTRAINT nutricion_registros_alumno_same_tenant_fkey FOREIGN KEY (tenant_id, alumno_id)
        REFERENCES public.alumnos(tenant_id, id) ON DELETE CASCADE;

ALTER TABLE public.workout_logs
    DROP CONSTRAINT IF EXISTS workout_logs_exercise_id_not_blank,
    DROP CONSTRAINT IF EXISTS workout_logs_day_index_non_negative,
    DROP CONSTRAINT IF EXISTS workout_logs_exercise_index_non_negative,
    DROP CONSTRAINT IF EXISTS workout_logs_set_index_non_negative,
    DROP CONSTRAINT IF EXISTS workout_logs_completion_consistency,
    DROP CONSTRAINT IF EXISTS workout_logs_alumno_same_tenant_fkey,
    DROP CONSTRAINT IF EXISTS workout_logs_rutina_same_tenant_fkey,
    DROP CONSTRAINT IF EXISTS workout_logs_alumno_id_fkey,
    DROP CONSTRAINT IF EXISTS workout_logs_rutina_id_fkey;

ALTER TABLE public.workout_logs
    ALTER COLUMN day_index SET NOT NULL,
    ALTER COLUMN exercise_index SET NOT NULL,
...
ALTER TABLE public.workout_logs
    ADD CONSTRAINT workout_logs_exercise_id_not_blank CHECK (btrim(exercise_id) <> ''),
    ADD CONSTRAINT workout_logs_day_index_non_negative CHECK (day_index >= 0),
    ADD CONSTRAINT workout_logs_exercise_index_non_negative CHECK (exercise_index >= 0),
    ADD CONSTRAINT workout_logs_set_index_non_negative CHECK (set_index >= 0),
    ADD CONSTRAINT workout_logs_completion_consistency CHECK (completed_at IS NULL OR completed = TRUE),
    ADD CONSTRAINT workout_logs_alumno_same_tenant_fkey FOREIGN KEY (tenant_id, alumno_id)
        REFERENCES public.alumnos(tenant_id, id) ON DELETE CASCADE,
    ADD CONSTRAINT workout_logs_rutina_same_tenant_fkey FOREIGN KEY (tenant_id, rutina_id)
        REFERENCES public.rutinas(tenant_id, id) ON DELETE CASCADE;
