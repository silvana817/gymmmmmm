...
## 7) Sobre logs `Shutdown reason: EarlyDrop` en Supabase

Si ves eventos `Boot` + `Shutdown` con `reason: EarlyDrop` y sin stacktrace, normalmente significa que la invocación se cortó antes de completar (cliente canceló, timeout de red, o request demasiado pesada/lenta), no necesariamente un crash interno de Deno.

Qué hacer:

1. Confirmá que el usuario esté autenticado (JWT válido) porque `verify_jwt = true`.
2. Usá imágenes más chicas (esta app ya las reduce automáticamente antes de enviar).
3. Revisá logs nuevos de la función (`[nutrition-analyzer] mode=...`) para ver tamaño de imagen, tiempo y cantidad de ítems.
4. Si persiste en móvil con mala red, reintentar con JPG y mejor conexión.
